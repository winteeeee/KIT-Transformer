# [KIT Transformer]
# 2. 배경
순차 계산을 줄인다는 아이디어는 확장 뉴런 GPU[16], ByteNet[18], ConvS2S[9]의 토대이며 이들 모두 기본 블록으로 컨볼루션 뉴런 네트워크를 사용하여 모든 입력과 출력 위치를 병렬적으로 계산합니다.
이러한 모델은 임의의 두 입력 혹은 출력 위치에서 관련된 신호를 연결하기 위한 작업 횟수는 위치 간 거리에 비례하여 ConvS2S에서는 선형적으로, ByteNet에서는 대수적으로 증가합니다. 
따라서 두 위치 사이의 거리가 멀수록 두 위치 사이에 존재하는 종속성을 학습하기 어려워집니다.[12] 
...
셀프 어텐션은 독해, 추상 요약, 문장 추론, 독립적 문장 표현 학습 등 여러 태스크에서 성공적이었습니다.[4, 27, 28, 22]

엔드-투-엔드 메모리 네트워크는 시퀀스 정렬 순환 대신 순환 어텐션 메커니즘을 기반으로 하며 간단한 QA와 언어 모델링 작업에서 우수한 성능을 보이는 것으로 나타났습니다.[34]

트랜스포머는 시퀀스 정렬 기반 RNN 혹은 컨볼루션을 사용하지 않고 모든 입력과 출력의 표현을 계산하기 위해 오직 셀프-어텐션만을 사용하는 최초의 변환 모델입니다. 
다음 절에서는 트랜스포머 모델과 셀프-어텐션에 대해 설명할 것이며 이러한 구조가 [17, 18, 9]에 비해 어떤 장점을 가지는지에 관해 논의합니다.
